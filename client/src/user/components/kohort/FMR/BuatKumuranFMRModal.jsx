...
export default function BuatKumuranFMRModel({
  setModalBuatKumuranFMR,
  singlePerson,
}) {
  ...
  const [tarikhKumuruanSemasa, setTarikhKumuruanSemasa] = useState('');
  const [tarikhKumuranSemasaDP, setTarikhKumuranSemasaDP] = useState(null);

  //   useEffect(() => {
  //     console.log(singlePerson);
  //   }, []);

  const TarikhKumusanSemasa = () => {
    return masterDatePicker({
      selected: tarikhKumuranSemasaDP,
      selectsStart: true,
      startDate: tarikhKumuranSemasaDP,
      required: true,
      onChange: (tarikhMula) => {
        const tempDate = moment(tarikhMula).format('YYYY-MM-DD');
        setTarikhKumuruanSemasa(tempDate);
        setTarikhKumuranSemasaDP(tarikhMula);
      },
      className:
