...
exports.isItOk = async (req, res) => {
    ...
    if (User.password === password) {
        // console.log(User);
        const genToken = jwt.sign({ userId: User._id, username: User.user_name, daerah: User.daerah, negeri: User.negeri }, process.env.JWT_SECRET, { expiresIn: '24h' });
...
exports.updateData = (req, res) => {
    if (req.params.id != 'pg') {
        Fasiliti.findByIdAndUpdate(req.params.id2, { $set: req.body }, function updateData(err) {
            if (err) { return next(err); }
            console.log('Updated ' + req.params.id2);
            res.redirect('/admin/' + req.params.id);
        });
    } else {
        Operator.findByIdAndUpdate(req.params.id2, { $set: req.body }, function updateData(err) {
            if (err) { return next(err); }
            console.log('Updated ' + req.params.id2);
            res.redirect('/admin/' + req.params.id);
        });
...
exports.listTaska = (req, res) => {
    ...
    Fasiliti.find({ jenisFasiliti: "Taska", daerah: jwt.verify(req.cookies.token, process.env.JWT_SECRET).daerah }, (err, data) => {
        if (err) {
            console.log(err);
        } else {
            res.render('admin/list', { data: data, fasiliti: 'Taska', fas: 'taska', mode: 'list' });
        }
    })
}
...
//   const daerah = "Kuala Selangor";
  Fasiliti.find({ jenisFasiliti: "Tadika", daerah: jwt.verify(req.cookies.token, process.env.JWT_SECRET).daerah }, (err, data) => {
    if (err) {
        ...
        res.render('admin/list', { data: data, fasiliti: 'Tadika', fas: 'tadika', mode: 'list' });
    }
})
}
...
//   const daerah = "Kuala Selangor";
  Fasiliti.find({ jenisFasiliti: "Klinik", daerah: jwt.verify(req.cookies.token, process.env.JWT_SECRET).daerah }, (err, data) => {
    if (err) {
        console.log(err);
    } else {
        res.render('admin/list', { data: data, fasiliti: 'Klinik', fas: 'kp', mode: 'list' });
    }
})
}
...
exports.listSr = (req, res) => {
  Fasiliti.find({ jenisFasiliti: "Sekolah Rendah", daerah: jwt.verify(req.cookies.token, process.env.JWT_SECRET).daerah }, (err, data) => {
    if (err) {
        console.log(err);
    } else {
        res.render('admin/list', { data: data, fasiliti: 'Sekolah Rendah', fas: 'sr', mode: 'list' });
    }
})
}
...
exports.listSm = (req, res) => {
  Fasiliti.find({ jenisFasiliti: "Sekolah Menengah", daerah: jwt.verify(req.cookies.token, process.env.JWT_SECRET).daerah }, (err, data) => {
    if (err) {
        console.log(err);
    } else {
        res.render('admin/list', { data: data, fasiliti: 'Sekolah Menengah', fas: 'sm', mode: 'list' });
    }
})
}
...
exports.listPg = (req, res) => {
    Operator.find({ daerah: jwt.verify(req.cookies.token, process.env.JWT_SECRET).daerah }, (err, data) => {
        if (err) {
            console.log(err);
        } else {
            res.render('admin/list', { data: data, fasiliti: 'Pegawai', fas: 'pg', mode: 'list' });
        }
    })
}
...
exports.searchAll = (req, res) => {
    ...
    console.log(regex);
    Fasiliti.find(
        { 'nama': { '$regex': regex } , daerah: jwt.verify(req.cookies.token, process.env.JWT_SECRET).daerah },
        function (err, data) {
            if (err) {
                console.log(err);
            } else {
    ...
    Operator.find(
        ...
        function (err, data) {
            if (err) {
                ...
                console.log(data);
                res.render('admin/list', { data: data, fasiliti: "Pegawai", mode: 'search' });
            }
        })
    }
...
exports.updateFac = (req, res) => {
    async.parallel({
        ...
        // console.log(results.listklinik);
        res.render('admin/update', { title: 'Entrails', data: results.fasilitiNow, klinik: results.listklinik  });
...
exports.updateNow = async (req, res) => {
    Fasiliti.findByIdAndUpdate(req.params.id, { handler: req.body.kpSkrg }, (err) => {
        if (err) {
            console.log(err);
        } else {
            setTimeout(function () {
