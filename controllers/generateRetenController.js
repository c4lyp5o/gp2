  ...
  try {
    ...
    switch (fromEtl) {
      case 'true':
        ...
        break;
      default:
        data = await Helper.countPPIM03SR(payload);
        break;
    }
    //
    if (data.length === 0) {
    ...
    for (let i = 0; i < data[0].length; i++) {
      ...
      if (data[0][i].queryPPIM03[0]) {
        ...
        if (i < 2) {
          row.getCell(9).value =
            data[0][i].queryPPIM03[0].jumlahMuridMelaksanakanBegin;
        }
        if (i > 2) {
          row.getCell(9).value =
            data[0][
              i
            ].queryPPIM03[0].jumlahMuridRisikoTinggiMelaksanakanBegin;
        }
      }
    }
